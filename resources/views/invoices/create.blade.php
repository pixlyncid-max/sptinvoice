...
<div x-data="invoiceForm()" class="bg-white shadow-sm rounded-lg border border-slate-200">
    <form action="{{ route('invoices.store') }}" method="POST">
        ...
        <div class="px-4 py-5 border-b border-slate-200 sm:px-6">
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                ...
                <div class="col-span-1">
                    ...
                    <input type="text" name="nomor_invoice" value="{{ old('nomor_invoice', $nomor) }}" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-slate-300 rounded-md py-2 px-3 border @error('nomor_invoice') border-red-500 @enderror" required>
                ...
                <div class="col-span-1">
                    ...
                    <select name="client_id" x-model="clientId" @change="fetchRateCards()" class="mt-1 block w-full py-2 px-3 border border-slate-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm @error('client_id') border-red-500 @enderror" required>
                        ...
                        <option value="{{ $client->id }}" {{ (old('client_id') ?? request('client_id')) == $client->id ? 'selected' : '' }}>{{ $client->nama }} {{ $client->perusahaan ? '('.$client->perusahaan.')' : '' }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-span-1">
                    ...
                    <input type="date" name="tanggal_invoice" value="{{ old('tanggal_invoice', now()->toDateString()) }}" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-slate-300 rounded-md py-2 px-3 border @error('tanggal_invoice') border-red-500 @enderror" required>
                ...
                <div class="col-span-1">
                    ...
                    <input type="date" name="tanggal_jatuh_tempo" value="{{ old('tanggal_jatuh_tempo', now()->addDays(30)->toDateString()) }}" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-slate-300 rounded-md py-2 px-3 border @error('tanggal_jatuh_tempo') border-red-500 @enderror" required>
                ...
                <div class="col-span-1">
                    ...
                    <input type="text" name="periode" value="{{ old('periode') }}" placeholder="Contoh: Juli 2026" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-slate-300 rounded-md py-2 px-3 border @error('periode') border-red-500 @enderror">
                ...
                <div class="col-span-3">
                    <div class="flex items-start mt-4">
                        <div class="flex h-6 items-center">
                            ...
                            <input id="dengan_ttd" name="dengan_ttd" type="checkbox" value="1" class="h-4 w-4 rounded border-slate-300 text-primary focus:ring-primary" checked>
                        </div>
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    <select x-model="item.deskripsi" 
                                        ...
                                        class="focus:ring-primary focus:border-primary block w-full sm:text-sm border-slate-300 rounded-md py-1 px-2 border" required>
        ...
        <div class="px-4 py-5 sm:px-6">
            ...
            <textarea name="catatan" rows="3" class="focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-slate-300 rounded-md py-2 px-3 border">{{ old('catatan') }}</textarea>
        </div>

        <!-- Submit -->
        <div class="px-4 py-4 bg-slate-50 text-right sm:px-6 border-t border-slate-200 rounded-b-lg">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            ...
                            <div class="mt-4">
                                <div class="mb-4 relative">
                                    <input type="text" x-model="searchRateCard" placeholder="Cari layanan atau kategori..." class="block w-full pl-10 pr-3 py-2 border border-slate-300 focus:outline-none focus:ring-primary focus:border-primary rounded-md shadow-sm sm:text-sm">
                                ...
                                <div x-show="rateCards.length === 0" class="text-center py-4 text-sm text-slate-500">
                                    Belum ada rate card aktif.
                                </div>
                                <div class="max-h-64 overflow-y-auto border border-slate-200 rounded-md">
                                    <ul class="divide-y divide-slate-200">
                                        <template x-for="rc in getFilteredRateCards()" :key="rc.id">
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('invoiceForm', () => ({
            ...
            items: [
                { id: Date.now(), deskripsi: '', qty: 1, harga_satuan: 0 }
            ],
            subtotal: 0,
            pajak_persen: 0,
            pajak_label: '{{ old('pajak_label', 'Pajak') }}',
            pajak_amount: 0,
            total: 0,
            ...
            init() {
                // Parse old input if validation failed
                @if(old('items'))
                    this.items = @json(old('items')).map(item => ({...item, id: Math.random()}));
                @endif
                
                @if(old('pajak_persen'))
                    this.pajak_persen = {{ old('pajak_persen') }};
                @endif
            ...
            addFromRateCard(rc) {
                ...
                if (this.items.length === 1 && this.items[0].deskripsi === '' && (this.items[0].harga_satuan == 0 || this.items[0].harga_satuan == null)) {
                    this.items.splice(0, 1, {...this.items[0], deskripsi: rc.nama_paket, harga_satuan: harga, qty: 1});
                } else {
