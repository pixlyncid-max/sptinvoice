        ...
        <div class="px-4 py-5 sm:p-6">
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div class="col-span-1">
                    <label class="block text-sm font-medium text-slate-700">Kode Barang (Otomatis)</label>
                    <div class="mt-1 flex rounded-md shadow-sm">
                        <input type="text" value="Dibuat Otomatis" class="focus:ring-primary focus:border-primary block w-full min-w-0 rounded-md sm:text-sm border-slate-300 py-2 px-3 border bg-slate-50 text-slate-600 font-mono font-bold cursor-not-allowed" readonly>
                    </div>
                    <p class="mt-1 text-xs text-slate-400">Kode barang unik (GAA-Kategori-Nomor-Tahun) akan di-generate otomatis saat disimpan.</p>
                </div>
                ...
                <div class="col-span-1">
                    ...
                    <select name="inventaris_category_id" id="inventaris_category_id" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-slate-300 rounded-md py-2 px-3 border bg-white @error('inventaris_category_id') border-red-500 @enderror" required>
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('inventaris_category_id') == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-span-1">
                    ...
                    <input type="text" name="nama_barang" id="nama_barang" value="{{ old('nama_barang') }}" placeholder="Contoh: Laptop ThinkPad X1" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-slate-300 rounded-md py-2 px-3 border @error('nama_barang') border-red-500 @enderror" required>
                    @error('nama_barang')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-span-1">
                    ...
                    <input type="text" name="nama_merk" id="nama_merk" value="{{ old('nama_merk') }}" placeholder="Contoh: Lenovo" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-slate-300 rounded-md py-2 px-3 border @error('nama_merk') border-red-500 @enderror">
                    @error('nama_merk')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-span-1">
                    ...
                    <input type="date" name="tanggal_beli" id="tanggal_beli" value="{{ old('tanggal_beli', date('Y-m-d')) }}" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-slate-300 rounded-md py-2 px-3 border @error('tanggal_beli') border-red-500 @enderror" required>
                    @error('tanggal_beli')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-span-1 sm:col-span-2">
                    ...
                    <select name="employee_id" id="employee_id" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-slate-300 rounded-md py-2 px-3 border bg-white @error('employee_id') border-red-500 @enderror">
                        <option value="">-- Belum Diserahkan (Opsional) --</option>
                    ...
                    @error('employee_id')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-slate-400">Hubungkan barang ini ke karyawan sebagai pengguna utama aset perusahaan.</p>
                </div>
