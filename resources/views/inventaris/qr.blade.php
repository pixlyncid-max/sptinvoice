...
@section('actions')
    <div class="flex gap-2">
        <a href="{{ route('inventaris.show', $inventaris) }}"
            class="inline-flex items-center justify-center px-4 py-2 border border-slate-300 text-sm font-medium rounded-md shadow-sm text-slate-700 bg-white hover:bg-slate-50 transition">
            Lihat Detail Barang
        </a>
...
@section('content')
    <div class="max-w-2xl mx-auto bg-white shadow-sm rounded-lg border border-slate-200 overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
            ...
            <span
                class="inline-flex items-center px-3 py-1 rounded-md text-sm font-mono font-bold bg-slate-200 text-slate-800 border border-slate-300">
        ...
        <div class="p-8 flex flex-col items-center text-center">
            ...
            <div class="mb-6">
                ...
                @if($inventaris->nama_merk)
                    <p class="text-sm text-slate-500 mt-1">Merk: <span
                            class="font-semibold text-slate-700">{{ $inventaris->nama_merk }}</span></p>
                @endif
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-md">
                <button onclick="printQRCodeLabel()"
                    class="inline-flex justify-center items-center gap-1.5 px-4 py-2.5 border border-transparent text-sm font-semibold rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-750 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition">
                ...
                <a href="{{ route('inventaris.qr-scan', $inventaris->kode_barang) }}" target="_blank"
                    class="inline-flex justify-center items-center gap-1.5 px-4 py-2.5 border border-slate-300 text-sm font-semibold rounded-md shadow-xs text-slate-700 bg-white hover:bg-slate-50 transition">
    ...
    <div id="print-label-section" class="hidden">
        ...
        <div
            ...
            <!-- Left: QR Code Box -->
            <div
                style="background: white; border-radius: 1mm; padding: 1mm; display: flex; align-items: center; justify-content: center; width: 21mm; height: 21mm; box-sizing: border-box; flex-shrink: 0; margin-right: 2.5mm;">
