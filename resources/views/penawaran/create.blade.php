...
<div x-data="penawaranForm()" class="bg-white shadow-sm rounded-lg border border-slate-200">
    <form action="{{ route('penawaran.store') }}" method="POST">
        ...
        <div class="px-4 py-5 border-b border-slate-200 sm:px-6">
            ...
            <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div class="sm:col-span-1">
                    ...
                    <input type="text" name="nomor_penawaran" value="{{ old('nomor_penawaran', $nomor) }}" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-slate-300 rounded-md py-2 px-3 border @error('nomor_penawaran') border-red-500 @enderror" required>
                ...
                <div class="sm:col-span-1">
                    ...
                    <input type="date" name="tanggal" value="{{ old('tanggal', now()->toDateString()) }}" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-slate-300 rounded-md py-2 px-3 border @error('tanggal') border-red-500 @enderror" required>
                ...
                <div class="sm:col-span-1">
                    ...
                    <input type="date" name="berlaku_hingga" value="{{ old('berlaku_hingga', now()->addDays(14)->toDateString()) }}" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-slate-300 rounded-md py-2 px-3 border @error('berlaku_hingga') border-red-500 @enderror" required>
        ...
        <div class="px-4 py-5 border-b border-slate-200 sm:px-6 space-y-8">
            ...
            <template x-for="(item, index) in getSubmittableItems()" :key="'submit-'+item.id">
                <div>
            ...
            <input type="hidden" name="pajak_persen" value="0">
            <input type="hidden" name="pajak_label" value="Pajak">
        </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('penawaranForm', () => ({
            ...
            items: [
                { id: 'initial-1', kategori_layanan: 'Jenis Pekerjaan', deskripsi: '', keterangan: '', has_keterangan: false, qty: 1, harga_satuan: 0 },
            ...
            init() {
                @if(old('items'))
                    this.items = @json(old('items')).map((item, idx) => ({
                        ...item, 
                        id: 'old-' + idx,
            ...
            getSubmittableItems() {
                return this.items.filter(item => {
                    ...
                    if (item.kategori_layanan === 'Data Yang Diperlukan') {
                        return item.deskripsi && item.deskripsi.trim() !== '';
                    }
                    return true;
                });
