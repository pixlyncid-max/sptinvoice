        ...
        Alpine.data('penawaranForm', () => ({
            ...
            items: {!! json_encode($penawaran->items->map(function($item) use ($rateCards) {
                ...
                return [
                    ...
                    'has_keterangan' => !empty($item->keterangan),
                    'qty' => (float) $item->qty,
                    'harga_satuan' => (float) $item->harga_satuan,
                    'is_custom' => $isCustom
                ];
            })) !!},
            useJenisPekerjaanTable: {{ $penawaran->items->where('kategori_layanan', 'Jenis Pekerjaan')->count() > 0 ? 'true' : 'false' }},
            useJenisPekerjaan2Table: {{ $penawaran->items->where('kategori_layanan', 'Jenis Pekerjaan 2')->count() > 0 ? 'true' : 'false' }},
            ...
            init() {
                if (this.items.length > 0) {
                    this.items = this.items.map(item => ({
                        ...item,
                        has_keterangan: item.has_keterangan !== undefined ? item.has_keterangan : !!(item.keterangan && item.keterangan.trim() !== '')
                    }));
                ...
                fetch(`{{ route('rate-cards.items') }}?divisi=${divisi}`, {
                    ...
                    .then(data => {
                        ...
                        this.items.forEach(item => {
                            if (item.kategori_layanan === 'Fee Pekerjaan' && item.deskripsi) {
                                const exists = this.rateCards.some(rc => rc.nama_paket === item.deskripsi);
                                if (!exists) {
                                    item.is_custom = true;
                                }
                            }
